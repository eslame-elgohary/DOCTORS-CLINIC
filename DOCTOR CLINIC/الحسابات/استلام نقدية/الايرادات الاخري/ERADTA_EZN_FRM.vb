Public Class ERADAT_EZN_FRM
    ...
    Private Sub ERADAT_EZN_FRM_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Sub FILL_MASROUF()
        ...
        Dim DT As New DataTable
        Dim DA As New SqlClient.SqlDataAdapter("SELECT * FROM ERADAT WHERE STAT='TRUE' ", SqlConn)
        DA.Fill(DT)
        For I = 0 To DT.Rows.Count - 1
            TXT_ACTION.Items.Add(DT.Rows(I).Item("ERADAT_NAME"))
        ...
        If DT0.Rows.Count > 0 Then
            If DT0.Rows(0).Item("O1").ToString = True Then
                ...
                If TOTAL.Text < 1 Then
                    MessageBox.Show("برجاء ادخال التوريد المراد حفظه ", "رسالة تنبية", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    TXT_ACTION.Select()
                    Exit Sub
                End If
                ...
                DA.Fill(DT)
                If DT.Rows.Count > 0 Then
                    MessageBox.Show("يوجد أيصال أخر لهذا الأيراد اليوم، يرجى التأكد", "رسالة تنبيه", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Else
                    '""""""""""""""" الترقيم التلقائي """"""""""""""
                    TXT_CODE.Text = CODE_GENE("OUT_MASROUF", "ID") + 1
                    '""""""""""""""""""""""""""""""
                ...
                DR2!KHAZINA_NAME_ACTION = "أيصال توريد نقدية رقم " & TXT_CODE.Text & " الأيرادات "
    ...
    Private Sub EDITBTN_Click(sender As Object, e As EventArgs) Handles EDITBTN.Click
        ...
        If DT0.Rows.Count > 0 Then
            If DT0.Rows(0).Item("O2").ToString = True Then
                ...
                CALC()
                If TOTAL.Text < 1 Then
                    MessageBox.Show("برجاء ادخال الأيرادات المراد حفظها ", "رسالة تنبية", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    TXT_ACTION.Select()
                    Exit Sub
                End If
                If Val(TXT_DAY.Text) >= Val(TXT_DAY2.Text) Then
                    MessageBox.Show("لا يمكن تعديل ايصال أيرادات مر عليه اكثر من يوم", "رسالة تنبية", MessageBoxButtons.OK, MessageBoxIcon.Error)
                    TXT_CODE.Select()
                    Exit Sub
                End If
                '================= تخزين بيانات الصنف في قاعدة البيانات =============
    ...
    Private Sub ACTIVE_DOCTOR_Click(sender As Object, e As EventArgs) Handles ACTIVE_DOCTOR.Click
        If TXT_ACTION_CODE.Text = "" Then
            MessageBox.Show("برجاء ادخال نوع التوريد ؟ ", "رسالة تنبية", MessageBoxButtons.OK, MessageBoxIcon.Error)
            TXT_ACTION.Select()
            Exit Sub
        End If
        If TXT_PRICE.Text = "" Then
            MessageBox.Show("برجاء ادخال قيمة التوريد ؟ ", "رسالة تنبية", MessageBoxButtons.OK, MessageBoxIcon.Error)
            TXT_PRICE.Select()
            Exit Sub
        End If
        ...
        If TXT_INFO_ACTION.Text = "" Then
            MessageBox.Show("برجاء ادخال تفاصيل التوريد ؟ ", "رسالة تنبية", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Private Sub TXT_ACTION_SelectedIndexChanged(sender As Object, e As EventArgs) Handles TXT_ACTION.SelectedIndexChanged
        Dim DT As New DataTable
        Dim DA As New SqlClient.SqlDataAdapter("SELECT * FROM ERADAT WHERE ERADAT_NAME='" & TXT_ACTION.Text & "'", SqlConn)
        DA.Fill(DT)
        For I = 0 To DT.Rows.Count - 1
            TXT_ACTION_CODE.Text = DT.Rows(I).Item("ERADAT_CODE")
        Next
    End Sub

    Private Sub ERADAT_EZN_FRM_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
